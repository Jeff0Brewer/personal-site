...
const Navbar = () => {
  ...
  const borderMin = '4px'; const borderMax = '12px'
  ...
  const updateNav = () => {
    ...
    for (let i = 0; i < sections.length; i++) {
      const rect = document.getElementById(sections[i]).getBoundingClientRect()
  ...
  return (
    <nav ref={navRef} className='navbar'>
      <motion.button
        className='home-nav' ref={tabRefs[0]}
      ...
      <motion.button
        className='about-nav' ref={tabRefs[1]}
      ...
      <motion.button
        className='work-nav' ref={tabRefs[2]}
      ...
      <motion.button
        className='contact-nav' ref={tabRefs[3]}
